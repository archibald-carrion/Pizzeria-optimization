...
path <- "C:\\Users\\PC\\Desktop\\SimulacionProyecto\\SimulacionDescuento\\pizza_sales.xlsx"
...
irregular_prop_buy <- 0.95  # Probabilidad de comprar cuando hay descuento de precios
discount_percentage <- 0.15 # porcentaje del aumento en el precio de las pizzas
...
simulate <- function(prop_buy, discount_percentage = 0) {
  ...
  for (day in 1:days_count) {
    ...
    if (possible_clients > 0) {
      ...
      for (i in 1:possible_clients) {
        ...
        # de que el cliente no la compre, ¿Por qué? Por que hubo un cambio de precio
        # entonces puede que cambie de opinión, se analiza este caso, si el cliente
        # decide comprarla se aplica el descuento del precio al income
        if (clients_orders[i] %in% top_pizzas$pizza_id) {
          if (runif(1) < prop_buy) {
            daily_income <- daily_income + current_pizza_prices[i]
            daily_income <- daily_income - (current_pizza_prices[i] * discount_percentage)
          }
        } else {
          # Sí no es una de las del top, es un posible cliente del historico, por
          # que si la compra. Además el caso a analizar es cuando hay descuento de en
          # las más vendidas
          daily_income <- daily_income + current_pizza_prices[i]
        }
      }
...
for (i in 1:simulation_number) {
  ...
  income_simulation_higher_price[i, ] <- simulate(prop_buy = irregular_prop_buy, discount_percentage = discount_percentage) 
}
...
# Labels del grafico
type_with_change <- paste0(discount_percentage * 100, "%")
label_without_change <- "Sin descuento"
label_with_change <- "Con descuento"
type_without_change <- "0%"

# Crear gráfico con leyendas ajustadas
ggplot(df_mean_income) +
  geom_line(aes(x = Day, y = income_regular_price, color = label_without_change, linetype = type_without_change), linewidth = 1) +
  geom_line(aes(x = Day, y = income_irregular_price, color = label_with_change, linetype = type_with_change), linewidth = 1) +
  theme_minimal() +
  labs(
    title = "Precio Base vs Descuento en las Top Pizzas",
    x = "Día",
    y = "Ingreso Promedio (en dólares)",
    color = "Simulación",  # Label para colores
    linetype = "Porcentaje de descuento"  # label para tipos de línea
  ) +
...
df_density <- data.frame(
  ...
  type = rep(c("Sin descuento", "Con descuento"), each = length(mean_regular_income))
)
...
ggplot(df_density, aes(x = mean, fill = type, color = type)) +
  ...
  theme_minimal() +
  labs(
    title = "Densidad de Ingresos Promedio (Sin y Con descuento)",
